...
class Player {
  ...
  isAdmin: boolean;
  id: string;
  sentSolution: boolean;
  isLastWinner: boolean;

  constructor(obj) {
    obj && (<any>Object).assign(this, obj);
  }

  static getPlayer(players, hash): Player {
    return players.filter(d => d.hash === hash)[0];
  }

  isPlayerInGame(rooms): boolean {
...
class Room {
  ...
  static getRoomWithPlayer(rooms, hash): Room {
    return rooms.filter(d => d.players.filter(p => p.hash === hash).length > 0)[0];
  }

  static getRoomByHash(rooms, hash): Room {
  ...
  isPlayerAnAdmin(hash): boolean {
    return this.players.filter(d => d.hash === hash).length > 0;
  }

  toDTO(players = true, solutions = false): Object {
    // copy object
